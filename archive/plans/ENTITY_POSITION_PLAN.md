...
## 19. Dynamic-collection member records

Moved. The whole subject — why these members have no position, why their stored name
is an invented form, what was already repaired, and what is to be done next — lives in
**`DYNAMIC_MEMBER_NAMING_PLAN.md`**, which is self-contained and is the document to act
on. Nothing on the subject is kept here.
