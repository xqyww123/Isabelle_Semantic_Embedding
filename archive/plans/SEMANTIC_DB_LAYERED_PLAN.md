...
| L20 | **No local tombstone clearing — anywhere, ever:** no post-release purge, no tombstone-clearing `prune` command. CI's export is the only place tombstones are dropped. *Scope note (2026-07-27): the prohibition is on CLEARING tombstones. The incremental-invalidation plan's `prune` subcommand (batch deletion of stale-generation persistent records) WRITES tombstones via the `cmd_remove` engine and clears none — it is outside this rule, not an exception to it.* | a local tombstone is either working (masking an old system layer) or inert (bounded, harmless) |
...
| `export(outdir)` — **lives in `snapshot_sync.py`** (approved 2026-07-26: producer, consumer, format constants and gates of the snapshot artifact share one module; name `export` replaces the earlier "flatten", whose layer-collapse connotation was vacuous on the real input) | The artifact producer, executed BY CI (L18). Six jobs over the input DB (on the runner: the HF cache's user DB, single-layer — iteration goes through the facade, which simply reads it straight): ① omit tombstoned keys everywhere (records AND their vectors; tombstones themselves never ship); ② drop every key whose theory-hash WIP bit is set, and the `0xF0` global-counter key (2026-07-27, from the incremental-invalidation plan §9: WIP records are machine-local working state whose name-hash keys are content-blind — shipped, they would mask real interpretations on consumer machines; the build machine's counter value is meaningless and harmful elsewhere; the HF source tarball does contain WIP keys today, §15.3, so this filter is live, not theoretical); ③ rebuild the experience index over the output (reusing the `rebuild_experience_index` core, parameterized by src/dst envs — the HF copy's live index is NOT shipped); ④ stamp `manifest.json` (`created_at` = build time, the L15 clock; store stats); ⑤ compacting rewrite (free pages dropped as a side effect of the key-by-key copy); ⑥ run the gates (`_check_manifest` self-check, `_check_no_legacy`, `_check_vector_format`). ~120–150 lines incl. plumbing. (CI invocation: the `export` subcommand, §15.3) |
...
5. export: output contains no tombstoned keys and no tombstones (records and
   vectors), no keys with the theory-hash WIP bit set, and no `0xF0`
   global-counter key; system experience index present and consistent with the
   output records; manifest stamped; gates run.
...
2026-07-26 (post-review): library release 0.3.0 approved; **the data package
gains its single run dependency `isabelle-semantic-embedding >=0.3.0`** (user
decision — closes the silent-skew hole where a pre-layered library ignores an
installed payload; supersedes the "dependency-less" wording in L3/§7);
`release` asks real-or-dry and passes the explicit `dry_run` flag (§14).
2026-07-27: export gains job ② — drop WIP-bit keys and the `0xF0`
global-counter key (from the incremental-invalidation plan §9, approved
2026-07-26; validation item 5 extended to match); L20 scope note added
(the prohibition covers tombstone CLEARING; the invalidation plan's
tombstone-WRITING `prune` is outside it).
