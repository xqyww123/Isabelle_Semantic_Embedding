...
fun galmuth :: "nat list => nat" where
  "galmuth [] = 0"
| "galmuth (x # xs) = (if x mod 2 = 1 then x + galmuth xs else galmuth xs)"
...
fun narquil :: "quilm => nat" where
  "narquil (Brint n) = (if n < 6 then 1 else 0)"
...
inductive plerx :: "nat => bool" where
  plerx_base: "plerx 0"
| plerx_step: "plerx n ==> 2 < n ==> plerx (n + 2)"
...
locale dremnok =
  fixes zug :: "'a => 'a => 'a"
  assumes zug_law: "zug (zug x y) y = x"
begin
