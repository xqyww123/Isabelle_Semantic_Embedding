...
# The agent echoes these labels back through the `answer` tool (its `type`
# enum is this table's values) and they are matched by `_label`.
_KIND_PROMPT_LABELS = {
...
class AgentTask:
    ...
    def __init__(self, connection: Connection, file_path: str,
        ...
        # D18: no stored interpretation of THIS theory's entities is disclosed
        # to the agent in this session, enrolled or not -- an entity enrolled
        # by a later CHANGED verdict may already have been read otherwise, and
        # an echoed text would make the judge say "same" on a real change.
        self.theory_keys: frozenset[bytes] = frozenset(e.universal_key for e in entries)
...
class InterpretationTask(AgentTask):
    ...
    def _collapse_provenance(prompt_extra: str,
        ...
        When this entry's "Generated by ..." head line is byte-identical to
        ``prev``'s (with the same 'Locale "...": ' description line, or none
        on both -- D18 omits that line whenever the locale is an entry of this
        run), the head is replaced by a back-reference and the description
        line, if any, is dropped.  The head names the locale, the qualifier
        and the whole instantiation, so it is the identity of the
        interpretation.  Returns the (possibly rewritten)
        ...
        if head is not None and (head, locale) == prev:
    ...
    def format_entries(self, indices: Iterable[int]) -> str:
        ...
        for i in indices:
            e = self.entries[i]
            line = (f"  [line {e.line_number}] " if e.line_number > 0 else "  ") + _label(e)
            if e.prop_str:
                line += f": {e.prop_str}"
            if e.prompt_extra:
        ...
        "interpretations": {
            ...
            "items": {
                ...
                "properties": {
                    "type": {
                        "type": "string",
                        "enum": list(_KIND_PROMPT_LABELS.values()),
                    },
                    "name": {
                        "type": "string",
...
def _statement(e: Entry, rec: 'SemanticRecord | None') -> 'str | None':
    """The statement text to store for `e` (D19): the wire `prop_str`, or the
    stored one when the wire's is "" -- `mk_prop_str` yields "" when the
    statement could not be computed (an ML_file method, an unreadable theory
    file), never for an entity that has none."""
    return e.prop_str or (rec.expr if rec is not None else e.prop_str)
...
def _refresh_statements(entries: list[Entry],
    ...
    write, a stored `expr` that differs from the wire statement is rewritten
    (and its vector tombstoned).  The one write a dry run makes."""
    for i in not_enrolled:
        e = entries[i]
        rec = rec_cache[e.universal_key]
        assert rec is not None, "not enrolled means filled from the store"
        expr = _statement(e, rec)
        if rec.expr != expr:
            Semantic_DB.update_expr(e.universal_key, expr)
...
async def _prefilter(task: InterpretationTask, e: Entry,
    ...
    texts -- the baseline and the fresh interpretation, each under the wire
    statement, rendered by the ONE document-text authority
...
def _write(task: InterpretationTask, idx: int, rec: 'SemanticRecord | None',
    ...
    with _note_on_failure(f"while writing the interpretation of {e.name}"), \
        ...
        written = w.put_interpretation(
            ...
            expr=_statement(e, rec), interpretation=text,
...
def _stop_if_cancelled() -> None:
    ...
    recycled session would have every gate refused and park for ever.  It
    raises a `CancelledError` (the arm's exception kept as its context), never
    that exception itself.  Also called first in `_judge`'s `except Exception`,
    which reads any session failure as CHANGED and would otherwise let the
    gate write and mint on a run being torn down -- whichever arm of the
    judge's `_run_agent` let the laundered exception out."""
    t = asyncio.current_task()
    if t is not None and t.cancelling():
        _log.info("agent: the session is being cancelled; its teardown raised "
                  "instead of the cancellation", exc_info=True)
...
async def interpret_file(
    ...
    # _statement(e, rec)` and put back into rec_cache, and `_statement` over
    # that written record returns the same string, so the guard is a no-op.
    _refresh_statements(entries, rec_cache, not_enrolled)
